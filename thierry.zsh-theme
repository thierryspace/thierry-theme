...
prompt_rsegment() {
  echo -n "  %F{$1}$RSEGMENT_SEPARATOR%F{$2}%K{$1}"
  [[ -n $3 ]] && echo -n $3
}
  ...
  local ref dirty mode repo_path
   ...
   if [[ "$(command git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    if [[ -n $dirty ]]; then
      ...
      prompt_rsegment green $CURRENT_FG
    fi
...
prompt_kubecontext() {
	local context="`kubectl config current-context`/`kubectl config get-contexts --no-headers | grep '*' | awk '{print $5}'`"
        if [[ $context == *"prod"* ]]; then
		prompt_segment magenta black $context
	else
...
prompt_time() {
	prompt_rsegment white black " %D{%H:%M} "
}
...
RPROMPT='$(build_rprompt)%K{$CURRENT_RBG}'
